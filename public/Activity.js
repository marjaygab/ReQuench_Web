$('document').ready(function () {
    ...
    fetchHistory(jsonobj.Account_Details.Acc_ID, function (temp_array) {
        ...
        if (temp_array.length > 0) {
            ...
            today = new moment();
            console.log('Date: ' + typeof(today) + ' Value: ' + today);
            a_week_date = moment(today, "YYYY-MM-DD").add(7, 'days');
            for (var i = 0; i < list.length; i++) {
                if (Date.parse(today.format('YYYY-MM-DD')) <= Date.parse(list[i].Date) && Date.parse(a_week_date.format('YYYY-MM-DD')) >= Date.parse(list[i].Date))
                    list[i].Visible = 'true';
                else
    ...
    $('input[name="daterange"]').daterangepicker({
        ...
        var temp_array = [];
        console.log('Start: ' + typeof(start) + ' Value: ' + start);
    ...
    $('input[name="daterange"]').data('daterangepicker').setEndDate(a_week_date);
});
